...
const expectedTitle = "David Bartlett Park Off-Leash Guide | Ottawa | LeashFree.ca";
const expectedDescription = "Plan a visit to David Bartlett Park in Manotick with its mixed leash boundary, official address, free gravel parking, hours and current Ottawa dog rules.";
const expectedCanonical = "https://leashfree.ca/dog-parks/david-bartlett-park/";
const expectedSource = "https://ottawa.ca/en/recreation-and-parks/parks-and-green-space/dog-parks/map-dogs-parks";
const expectedReviewed = "2026-08-28T19:00:00.000Z";
const expectedAlt = "Realistic digital illustration of two off-leash dogs and their handler in a tree-lined clearing at David Bartlett Park in Manotick";
...
if (markVerification) {
  ...
  console.log("Marked David Bartlett Park verification-pending."); process.exit(0);
}
...
check(visibleText.includes("Information reviewed Aug 28, 2026"), "Reviewed On text missing"); check(visibleText.includes("Most of the park is off leash, with one leashed section") && visibleText.includes("south of the fence"), "mixed leash boundary identity missing");
check(visibleText.includes("5201 McLean Crescent") && visibleText.includes("Manotick"), "confirmed address or community missing");
check(visibleText.includes("approximately 14.285 hectares") && visibleText.includes("whole park"), "confirmed full-park area context missing");
check(visibleText.includes("free 25-space gravel") && visibleText.includes("no lighting") && visibleText.includes("no designated accessible spaces"), "confirmed parking details missing");
check(visibleText.includes("no waterbody access") && visibleText.includes("dog-swimming area"), "waterbody restriction missing");
check(visibleText.includes("Potable water") && visibleText.includes("washrooms") && visibleText.includes("not currently published"), "unknown visitor details disclosure missing");
check(visibleText.includes("in sight and under voice control") && visibleText.includes("annual dog registration") && visibleText.includes("5 a.m. to 11 p.m."), "Ottawa dog or park rules missing");
check(!visibleText.includes("K4M1G2") && !visibleText.includes("seasonal washroom") && !visibleText.includes("Rideau River access for dogs"), "unsupported legacy facts visible");
...
check(Boolean(breadcrumbSchema), "Breadcrumb schema missing"); check(Boolean(parkSchema), "Park schema missing"); check(parkSchema?.name === "David Bartlett Park", "Park schema name mismatch");
check(parkSchema?.address?.streetAddress === "5201 McLean Crescent" && parkSchema?.address?.addressLocality === "Ottawa" && parkSchema?.address?.addressRegion === "Ontario" && parkSchema?.address?.postalCode === undefined, "Park schema address mismatch");
check(String(parkSchema?.geo?.latitude) === "45.24474882" && String(parkSchema?.geo?.longitude) === "-75.69989422", "Park schema coordinates mismatch"); check(parkSchema?.dateModified === expectedReviewed, "Park schema Reviewed On mismatch");
check(Array.isArray(parkSchema?.amenityFeature) && parkSchema.amenityFeature.length === 1 && parkSchema.amenityFeature.some((item) => item.name === "Parking"), "Park schema amenity set mismatch");
check(Boolean(faqSchema), "FAQ schema missing"); check(faqSchema?.dateModified === expectedReviewed, "FAQ schema Reviewed On mismatch"); check(faqSchema?.mainEntity?.length === 9, "FAQ schema count mismatch"); check(html.includes(`href="${expectedSource}"`), "official source link missing");

check(html.includes(imageSource), "intended source image missing"); check(html.includes(`alt="${expectedAlt}"`), "hero alt mismatch");
for (const width of [480, 960]) for (const format of ["avif", "webp", "jpg"]) { const derivative = `/images/optimized/dog-parks/david-bartlett-park-original/${width}.${format}`; check(html.includes(derivative), `rendered derivative missing: ${width}.${format}`); check(fs.existsSync(`public${derivative}`), `derivative file missing: ${width}.${format}`); }
...
check(park?.canonicalUrl === expectedCanonical, "generated JSON canonical mismatch"); check(park?.raw?.["Park Website or Source"] === expectedSource, "generated JSON source mismatch"); check(park?.raw?.City === "Ottawa" && park?.raw?.Province === "Ontario", "JSON locality mismatch");
check(park?.raw?.["Street Address"] === "5201 McLean Crescent" && park?.raw?.latitude === "45.24474882" && park?.raw?.longitude === "-75.69989422" && park?.raw?.["Postal Code"] === "" && park?.raw?.["Google Maps Link"] === "", "JSON location fields mismatch");
check(park?.raw?.["Reviewed On"] === "Fri Aug 28 2026 19:00:00 GMT+0000 (Coordinated Universal Time)", "generated JSON Reviewed On mismatch"); check(park?.raw?.["Updated On"] === "Tue Jun 24 2025 21:04:14 GMT+0000 (Coordinated Universal Time)", "Webflow Updated On unexpectedly changed");
...
if (failures.length) { console.error(`David Bartlett Park audit failed (${failures.length}):`); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
if (finalize) updateManifest({
  ...
  image: { source: imageSource, width: 1536, height: 1024, compressedBytes: fs.statSync(imagePath).size, alt: expectedAlt, derivatives: [480, 960], formats: ["avif", "webp", "jpg"], reuse: "unique to David Bartlett Park", sha256: sourceHash, generation: "Built-in ImageGen on 2026-08-28; independent original with no reference image", visualReference: "Confirmed City context only: a broad community park beside the Rideau River where most parkland is off leash. Two dogs and one attentive handler remain well away from the vegetated shoreline; the illustration adds no fence, sign, water access or unconfirmed amenity and is not presented as a photograph." },
  reason: "Completed with current City of Ottawa park inventory, dog-designation, parking, park-hours, animal-control and registration sources; synchronized visitor copy, SEO, FAQs, generated JSON and CMS CSV; verified the unique compressed illustration and six responsive derivatives; completed production build, repository page QA, exact parity, and rendered metadata, schema, source, image, fallback and editorial-marker inspection.",
  nextAction: "Complete; proceed to Guindon Park on the next page-improvement run."
}, "Guindon Park");
console.log(`David Bartlett Park audit passed (${finalize ? "finalized" : "verification only"}); SHA-256 ${sourceHash}.`);
