...
const imageSource = "/images/dog-parks/sunnidale-park-original.png";
const sourceImagePath = `public${imageSource}`;
const expectedTitle = "Sunnidale Park DOLRA | Barrie, Ontario | LeashFree.ca";
const expectedDescription = "Plan a visit to Sunnidale Park DOLRA in Barrie: a secure fenced natural area with woodland footpaths, parking, current rules, and safety cautions.";
const expectedCanonical = "https://leashfree.ca/dog-parks/sunnidale-park/";
const expectedSource = "https://www.barrie.ca/community-recreation-environment/parks-trails-waterfront/dog-leash-recreation-areas";
const expectedAlt = "Painterly illustration of two off-leash dogs and their handler on a fenced woodland footpath at Sunnidale Park in Barrie";
const expectedReviewed = "2026-08-26T12:00:00.000Z";
const expectedCoordinates = { latitude: "44.3969708514", longitude: "-79.7013663948" };
...
function updateManifest(patch, nextPriorityPage = "Sunnidale Park") {
...
if (markImplementation) {
  updateManifest({
    ...
    artifactChecks: implementationChecks,
    nextAction: "Mark verification pending, then run the full production and rendered-page QA."
  });
  console.log("Marked Sunnidale Park implementation-complete.");
  process.exit(0);
}
...
if (markVerification) {
  ...
  console.log("Marked Sunnidale Park verification-pending.");
  process.exit(0);
}
...
check(visibleText.includes("227 Sunnidale Rd"), "address missing");
check(visibleText.includes("Barrie") && visibleText.includes("Ontario"), "city or province missing");
check(visibleText.includes("secure fenced natural area"), "confirmed fencing copy missing");
check(visibleText.includes("natural area with footpaths"), "confirmed surface copy missing");
check(visibleText.includes("Poison ivy"), "poison-ivy caution missing");
check(visibleText.includes("not monitored for water quality"), "stream-water caution missing");
check(visibleText.includes("No more than two dogs"), "two-dog rule missing");
check(!visibleText.includes("6 am") && !visibleText.includes("6 a.m. – 10 p.m."), "stale operating hours visible");
check(!visibleText.includes("L4M3B9") && !visibleText.includes("L4M 3B9"), "unsupported postal code visible");
...
check(parkSchema?.name === "Sunnidale Park", "Park schema name mismatch");
...
check(parkSchema?.address?.streetAddress === "227 Sunnidale Rd", "Park schema address mismatch");
check(parkSchema?.address?.addressLocality === "Barrie", "Park schema city mismatch");
...
check(amenities.has("Fenced area") && amenities.has("Waste bins") && amenities.has("Parking"), "confirmed amenity schema mismatch");
check(Boolean(faqSchema), "FAQ schema missing");
check(faqSchema?.dateModified === expectedReviewed, "FAQ schema Reviewed On mismatch");
check(faqSchema?.mainEntity?.length === 6, "FAQ schema count mismatch");
...
for (const width of [480, 960, 1600]) {
  for (const format of ["avif", "webp", "jpg"]) {
    const derivative = `/images/optimized/dog-parks/sunnidale-park-original/${width}.${format}`;
...
check(!html.includes("city-barrie-hero"), "unintended city fallback image used");
...
check(metadata.width === 1693 && metadata.height === 929, "source image dimensions mismatch");
...
if (failures.length) {
  console.error(`Sunnidale Park audit failed (${failures.length}):`);
