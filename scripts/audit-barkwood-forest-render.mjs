...
const expectedTitle = "Barkwood Forest Private Dog Park Guide | LeashFree.ca";
const expectedDescription = "Plan a Barkwood Forest visit in rural Stittsville: member screening, fenced 100-acre trails, dawn-to-dusk access, winter routes and dog rules.";
const expectedCanonical = "https://leashfree.ca/dog-parks/barkwood-forest/";
const expectedSource = "https://boogity.ca/barkwood-forest/";
const expectedReviewed = "2026-08-28T12:00:00.000Z";
const expectedAlt = "Realistic digital illustration of two off-leash dogs and their handler on a forest-and-meadow trail at Barkwood Forest";
...
if (markVerification) {
  ...
  console.log("Marked Barkwood Forest verification-pending."); process.exit(0);
}
...
check(visibleText.includes("Information reviewed Aug 28, 2026"), "Reviewed On text missing"); check(visibleText.includes("Private Off-Leash Dog Park") && visibleText.includes("private off-leash property") && visibleText.includes("screened members"), "private member-access identity missing");
check(visibleText.includes("100-acre fenced property") && visibleText.includes("evergreen forest") && visibleText.includes("roots and rocks"), "confirmed size or trail context missing");
check(visibleText.includes("daily from dawn until dusk") && visibleText.includes("plowed and packed by ATV"), "confirmed hours or winter maintenance missing");
check(visibleText.includes("open-house walk") && visibleText.includes("social behaviour") && visibleText.includes("off-leash recall"), "screening requirements missing");
check(visibleText.includes("two-gate airlock") && visibleText.includes("Garbage receptacles") && visibleText.includes("washing station"), "confirmed practical facilities missing");
check(visibleText.includes("registered annually") && visibleText.includes("City-issued tag"), "Ottawa registration rule missing");
check(visibleText.includes("does not publish an exact visitor street number, postal code or map coordinate"), "unknown exact location disclosure missing");
check(!visibleText.includes("7635 Fernbank") && !visibleText.includes("7580 Fernbank") && !visibleText.includes("K2S1B6") && !visibleText.includes("45.206") && !visibleText.includes("-75.978"), "disputed legacy location data visible");
check(!/https?:\/\//i.test(visibleText), "raw URL visible in page text");
check(!/(primary sources|research notes|implementation|verification|visitor-facing|editorial|source conflict|old copy|previously attached|improved page|built-in imagegen|route should|this page keeps|reviewed operator sources)/i.test(visibleText), "internal process language visible");

check(Boolean(breadcrumbSchema), "Breadcrumb schema missing"); check(!parkSchema, "Park schema should be omitted when coordinates are unknown"); check(!html.includes('"@type":"GeoCoordinates"'), "GeoCoordinates schema should be omitted when coordinates are unknown");
...
for (const width of [480, 960]) for (const format of ["avif", "webp", "jpg"]) { const derivative = `/images/optimized/dog-parks/barkwood-forest-original/${width}.${format}`; check(html.includes(derivative), `rendered derivative missing: ${width}.${format}`); check(fs.existsSync(`public${derivative}`), `derivative file missing: ${width}.${format}`); }
...
check(park?.raw?.latitude === "" && park?.raw?.longitude === "" && park?.raw?.["Postal Code"] === "" && park?.raw?.["Google Maps Link"] === "", "unknown location fields should be blank");
check(park?.raw?.["Reviewed On"] === "Fri Aug 28 2026 12:00:00 GMT+0000 (Coordinated Universal Time)", "generated JSON Reviewed On mismatch"); check(park?.raw?.["Updated On"] === "Tue Jun 24 2025 21:07:13 GMT+0000 (Coordinated Universal Time)", "Webflow Updated On unexpectedly changed");
...
if (failures.length) { console.error(`Barkwood Forest audit failed (${failures.length}):`); for (const failure of failures) console.error(`- ${failure}`); process.exit(1); }
